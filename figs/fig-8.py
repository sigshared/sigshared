...
def coleta_dados(fileName):
    ...
    with open(fileName) as fp:
      ...
      new_timestamp = timestamp_dpdk + 60
  
      while timestamp_dpdk < new_timestamp:
        line = fp.readline()
        timestamp_dpdk = int(float(line.split(";")[0]))
...
def draw_cdf(p_sig, p_sig99, p_sp, p_sp99):
    ...
    sigshared99 = ax.axvline(p_sig99  , marker = '' , linewidth=5, linestyle="-", label=f"99th sigshared = {p_sig99:.2f}",   color='tab:blue')
    spright99   = ax.axvline(p_sp99   , marker = '' , linewidth=5, linestyle="-", label=f"99th spright = {p_sp99:.2f}",   color='tab:red')


    cdf_sig = ax.plot(p_sig, percentiles, marker  = '',  ms = 1, mew = 2, label = 'SigShared', linewidth=7, linestyle="-",  color='tab:blue')
    cdf_sp  = ax.plot(p_sp , percentiles, marker  = '',  ms = 1, mew = 2, label = 'Spright', linewidth=7, linestyle="-",  color='tab:red')
    ...
    plt.savefig('fig-8.pdf' , dpi=figure.dpi,pad_inches=0.01)
...
if __name__ == '__main__':
    ...
    draw_cdf(p_sig[10], p_sig99[10], p_sp[10], p_sp99[10])
